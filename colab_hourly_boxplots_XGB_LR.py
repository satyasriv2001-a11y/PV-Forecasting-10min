...
# 5. Run Hourly Error Box Plots on Project1140 with both XGB and LR models
# This will process only Project1140.csv
# Results save directly to Drive!
# Generates box plots showing average error distribution by prediction start hour (0-23) for hourly resolution
...
DRIVE_PATH = "/content/drive/MyDrive/Solar PV electricity/hourly_boxplots_XGB_LR"
...
if len(project1140_files) == 0:
    ...
    print("=" * 80)
    print("Running Hourly Error Box Plots on Project1140")
    print("=" * 80)
    print(f"Configuration: XGBoost and Linear Regression, high complexity, PV+NWP, 24h lookback, NO time encoding")
    ...
    for file_idx, data_path in enumerate(project1140_files, 1):
        ...
        output_dir = os.path.join(DRIVE_PATH, plant_name)
        os.makedirs(output_dir, exist_ok=True)
        
        print(f"Output: {output_dir}")
        
        # Get the script path
        script_path = os.path.join(os.getcwd(), 'hourly_boxplots_XGB_LR.py')
        ...
        # Run hourly box plots for both XGB and LR
        cmd = [
            'python', script_path,
            '--data-path', data_path,
            '--complexity', 'high',
    ...
    print(f"\n{'='*80}")
    print("[SUCCESS] Hourly Error Box Plot Generation Completed!")
    print(f"Results saved to: {DRIVE_PATH}")
    print(f"{'='*80}")
    print("\nGenerated files for each model (XGB and LR):")
    print("  - error_boxplot_hourly_by_hour.png (Error box plot by hour 0-23 for hourly resolution)")
    print("  - average_error_hourly_XGB.csv (Summary table for XGB)")
    print("  - average_error_hourly_LR.csv (Summary table for LR)")
