...
# -*- coding: utf-8 -*-
"""
Run XGB and LR experiments for plant 1140 with dates 2022-03-12 to 2022-03-13
"""
...
# =============================================================================
# CONFIG GENERATION - Only XGB and LR
# =============================================================================
def generate_xgb_lr_configs():
    """
    Generate XGB and LR experiment configurations
    Dates: 2022-03-12 to 2022-03-13
    """
    configs = []
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_path = os.path.join(script_dir, "data", "Project1140.csv")

    # Only XGB from ML models
    ml_models = ['XGB']
    ...
    # === 1. XGB models: PV-based experiments ===
    ...
    # === 2. XGB models: NWP-only experiments ===
    for model in ml_models:
        for complexity in complexities:
    ...
    print(f"  XGB models: {1 * 2 * (16 + 4)} experiments = 40")
    print(f"  Linear model: 4 experiments")
    print(f"  Total: {len(configs)} experiments")
    print(f"  Date range: 2022-03-12 to 2022-03-13")
    return configs
...
def create_config(data_path, model, complexity, lookback, feat_combo, use_te, is_nwp_only):
    config = {
        ...
        'future_hours': 24,
        'start_date': '2022-03-12',  # Modified date range
        'end_date': '2022-03-13',    # Modified date range
        'save_options': {
            'save_model': False,
...
# =============================================================================
def run_experiments(output_dir=None):
    """
    Run XGB and LR experiments with dates 2022-03-12 to 2022-03-13
    
    Args:
        output_dir: Directory to save results (default: current directory)
    """
    print("=" * 80)
    print("PV Forecasting: Running XGB and LR Experiments")
    print("Plant: 1140")
    print("Date Range: 2022-03-12 to 2022-03-13")
    print("=" * 80)

    all_configs = generate_xgb_lr_configs()
    ...
    output_file = os.path.join(output_dir, "xgb_lr_1140_mar12_13_results.csv")
    ...
    for idx, config in enumerate(all_configs, 1):
        ...
        try:
            ...
            # Sequential split (no shuffle) for temporal evaluation
            ...
            # Both XGB and Linear use train_ml_model
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run XGB and LR experiments for plant 1140 (dates: 2022-03-12 to 2022-03-13)')
